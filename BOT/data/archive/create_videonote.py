...
class VideoNote:
    ...
    def __init__(self, telegram_id, video_note_time, msg_id, repeats=5):
        self.msg_id = msg_id
        ...
        try:
            class MyBarLogger(ProgressBarLogger):
                ...
                def bars_callback(self, bar, attr, value, old_value=None):
                    ...
                    if percentage == 25 and self.bar25 is False:
                        self.bot.edit_message_text(chat_id=self.USER_ID, text=f'⌛️ <i>Создаю ваш кружочек</i>... <b>{str(round(percentage))} %</b>', message_id=self.MSG_ID, parse_mode='HTML')
                        self.bar25 = True
                    if percentage == 50 and self.bar50 is False:
                        self.bot.edit_message_text(chat_id=self.USER_ID, text=f'⌛️ <i>Создаю ваш кружочек</i>... <b>{str(round(percentage))} %</b>', message_id=self.MSG_ID, parse_mode='HTML')
                        self.bar50 = True
                    if percentage == 75 and self.bar75 is False:
                        self.bot.edit_message_text(chat_id=self.USER_ID, text=f'⌛️ <i>Создаю ваш кружочек</i>... <b>{str(round(percentage))} %</b>', message_id=self.MSG_ID, parse_mode='HTML')
                        self.bar75 = True
